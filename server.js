...
// Disappearing-messages modes a chat (DM or group) can be set to. There is
// deliberately no "off"/permanent mode — messages are never kept around
// forever, only for as long as one of these modes allows. 'instant' (wiped
// the moment it's been shown once) is the default for new DMs and groups.
// The server only needs to know about this for groups — see the
// "disappearing" field on group objects below. For DMs it's a purely
// local, per-user display preference the client keeps to itself, so the
// server never sees it and this list is only consulted when validating
// the group endpoint.
const DISAPPEARING_MODES = ['instant', 'on-close', '24h', '7d'];
...
app.post('/api/groups', auth, async (req, res) => {
  ...
  try {
    const { error, group } = await transact(async (db) => {
      ...
      const id = crypto.randomUUID();
      const g = { id, name: trimmedName, owner: req.username, members: allMembers, disappearing: 'instant', createdAt: Date.now() };
      db.groups[id] = g;
      for (const m of allMembers) {
