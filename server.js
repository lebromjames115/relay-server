...
// Rename a group. Any current member can rename it — same permission model
// as adding a member above (there's no separate "owner-only" tier here).
// Everyone else gets a group-update notification so their local copy (and
// future fan-out sends, which key off name only for display) stays in sync.
app.post('/api/groups/:id/rename', auth, async (req, res) => {
  const { name } = req.body || {};
  const trimmedName = typeof name === 'string' ? name.trim() : '';
  if (!trimmedName || trimmedName.length > 80) {
    return res.status(400).json({ error: 'Group name must be 1-80 characters.' });
...
// Leave a group. Self-service only — there's no "remove another member"
// endpoint, by design. If this was the last member, the group is deleted
// outright rather than left behind as an orphaned, memberless row.
...
// Removes a contact from the caller's own list only — this is a one-sided,
// per-user list (see the comment above), so there's nothing to notify the
// other person about and no membership to validate.
app.delete('/api/contacts/:username', auth, async (req, res) => {
  await transact((db) => {
    if (db.contacts[req.username]) {
      db.contacts[req.username] = db.contacts[req.username].filter((c) => c !== req.params.username);
    }
  });
  return res.json({ ok: true });
});
